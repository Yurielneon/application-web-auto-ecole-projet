...
import Image from "next/image"; // Ajout de l'import pour Image
...
// URL de base pour les images depuis l'API Laravel
...
const StudentManager: React.FC = () => {
    ...
    const actions = [
        { icon: <FaEye />, onClick: (row: any) => handleView(row), tooltip: "Voir" },
    ...
    const validateCategoryPrerequisites = () => {
        if (!formData.training_id || !formData.previous_license) {
            return true;
        }
        const training = trainings.find((t) => t.id === parseInt(formData.training_id));
        if (!training || !training.category_id) {
            return true;
        }
    ...
    const handleEditStudent = async () => {
        ...
        Object.entries(formData).forEach(([key, value]) => {
            if (value !== null && value !== '') {
                if (key === 'profile_picture' && value === null && selectedStudent.profile_picture) {
                    return;
                }
                if (key === 'residence_certificate' && value === null && selectedStudent.residence_certificate) {
                    return;
                }
                if (key === 'payment_receipt' && value === null && selectedStudent.payment_receipt) {
                    return;
                }
    ...
    return (
        <div className="p-6">
            ...
            {loading ? (
                ...
                <DataTable columns={columns} data={validatedStudents} actions={actions} />
            )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Select
                            ...
                            onValidationChange={(isValid) => setFormValidation((prev) => ({ ...prev, training_id: !!value }))}
                    ...
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                ...
                                setFormValidation((prev) => ({ ...prev, profile_picture: !!file }));
                            }}
                            required
                            shape="square"
                        />
                    ...
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                ...
                                setFormValidation((prev) => ({ ...prev, residence_certificate: !!file }));
                            }}
                            required
                            shape="square"
                        />
                    ...
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                ...
                                setFormValidation((prev) => ({ ...prev, payment_receipt: !!file }));
                            }}
                            required
                            shape="square"
                        />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <Input
                            ...
                            errorMessage="Le CIN doit contenir entre 6 et 12 caractères alphanumériques."
                        />
                    </div>
                    <div>
                        <Label htmlFor="birth_date">Date de naissance</Label>
                    ...
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                setFormData({ ...formData, profile_picture: file });
                                setFormValidation((prev) => ({ ...prev, profile_picture: true }));
                            }}
                            shape="square"
                        />
                    </div>
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                setFormData({ ...formData, residence_certificate: file });
                                setFormValidation((prev) => ({ ...prev, residence_certificate: true }));
                            }}
                            shape="square"
                        />
                    </div>
                    <div>
                        ...
                        <ImageUpload
                            onImageSelect={(file) => {
                                setFormData({ ...formData, payment_receipt: file });
                                setFormValidation((prev) => ({ ...prev, payment_receipt: true }));
                            }}
                            shape="square"
                        />
                    </div>
                ...
                {selectedStudent && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <Input id="view_gender" value={selectedStudent.gender === "1" ? "Homme" : selectedStudent.gender === "2" ? "Femme" : ""} disabled />
                        </div>
                        <div>
                            <Label htmlFor="view_training_id">Formation</Label>
                            <Input id="view_training_id" value={trainings.find(t => t.id === selectedStudent.training_id)?.title || ""} disabled />
                        </div>
                        <div>
                            <Label htmlFor="view_previous_license">Licence précédente</Label>
                            <Input id="view_previous_license" value={categories.find(c => c.id === parseInt(selectedStudent.previous_license))?.name || ""} disabled />
                        </div>
                        <div>
